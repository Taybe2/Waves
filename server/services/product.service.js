...
cloudinary.config({
    cloud_name: 'dhbzdiro8', 
    api_key: '449872771585768', 
    api_secret: `${process.env.CN_API_SECRET}` 
})

const createProduct = async(body) => {
    try {
        const productExits = await Product.findOne({model: body.model});

        if(productExits) throw new ApiError(httpStatus.BAD_REQUEST, "Product already exists")
...
const allProducts = async(req) => {
    try {
        ...
        if(!products) throw new ApiError(httpStatus.NOT_FOUND, "No products not found")
...
const picDelete = async(id) => {
    console.log(id)
    try {
        const upload = await cloudinary.uploader.destroy(id);
